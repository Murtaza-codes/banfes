...
def quiz_list(request, slug):
    ...
    return render(
        request, "quiz/quiz_list.html", {"quizzes": quizzes, "course": course, "active_page": "quizzes"}
    )
...
class QuizUserProgressView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Get or create the progress object with a single database query
        progress, _ = Progress.objects.get_or_create(user=self.request.user)
        
        # Use cached property to avoid repeated method calls
        context["cat_scores"] = progress.list_all_cat_scores
        
        # Optimize exams query using select_related to reduce database hits
        ...
        # Store exams in context
        context["exams"] = exams
        
        # Use cached queryset length instead of making a new query
...
class QuizTake(FormView):
    ...
    def final_result_user(self):
        ...
        # Only delete the sitting if it's not an exam paper AND the user is not a superuser or lecturer
        if (
            not self.quiz.exam_paper
            and not self.request.user.is_superuser
            and not self.request.user.is_lecturer
        ):
            self.sitting.delete()
...
@login_required
def fix_quiz_records_view(request):
    try:
        from .fix_quiz_records import fix_quiz_records
        result = fix_quiz_records()
        messages.success(request, f"Quiz records fix: {result}")
    except Exception as e:
        messages.error(request, f"Error fixing quiz records: {str(e)}")
