...
def generate_student_id():
    # Generate a full UUID for complete uniqueness
    unique_id = uuid.uuid4().hex 
    
    registered_year = datetime.now().strftime("%Y")
    
    # Remove the students_count dependency (avoiding race conditions)
    
    return f"{settings.STUDENT_ID_PREFIX}-{registered_year}--{unique_id[:10]}"  
