...
def home_view(request):
    # For non-logged in users, only show public news
    if request.user.is_authenticated:
        items = NewsAndEvents.objects.all().order_by("-updated_date")
    else:
        items = NewsAndEvents.objects.filter(is_public=True).order_by("-updated_date")
    ...
    if request.user.is_authenticated:
        user = request.user
        context["is_lecturer"] = user.is_lecturer
...
# New view functions for the pages
def news_view(request):
    ...
    if request.user.is_authenticated:
        ...
        items = NewsAndEvents.objects.filter(is_public=True).order_by('-updated_date')
    return render(request, 'core/news.html', {'items': items})

def news_detail_view(request, pk):
    item = get_object_or_404(NewsAndEvents, pk=pk)
    # If the post is not public and user is not authenticated, don't allow access
    if not item.is_public and not request.user.is_authenticated:
        messages.error(request, _("This content is only available to registered users."))
